...
void jiefuchuli(char* p) {      //界符处理函数,注释处理
	int j = jiefuornot(*p);
	int m;
	int t1,t2;
	int i=0;
   ...
   {  chuli[i]=*p;
          p++;
          i++;
          while((jiefuornot(*p)!=11)&&(jiefuornot(*(p--))!=5))
      {   chuli[i] = *p;
		  i++;
		  p++;
		  index++;
...
int VARIABLE()
{
	...
	{
	    if ((token[ip].content[0] == 'p') && (token[ip].code == 1))//p2是冒号
		{
			//func7();
			ip++;
		}
		...
		if ((token[ip].content[0] == 'p') && (token[ip].code == 2))//p3是分号
		{
			ip++;
		}
		else
...
int ANOUNCE_FUNCTION()
{
	...
	if ((token[ip].content[0] == 'p') && (token[ip].code == 0))/*是：*/
	{
		ip++;
	}
	else
	...
	if ((token[ip].content[0] == 'p') && (token[ip].code == 16))//是{
	{
		ip++;
	}
	else
	...
	if ((token[ip].content[0] == 'p') && (token[ip].code == 17))//是}
	{
		ip++;
	}
	else
...
int SEN_SEQUENCE()
{
	...
	while (((token[ip].content[0]=='p')&&(token[ip].code==2)))//p3是分号
	{
		ip++;
		if (EVA_SENTENCE()==0)
		{
